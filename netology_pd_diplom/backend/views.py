    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'password' in request.data:
            ...
            try:
                ...
                for item in password_error:
                    error_array.append(item)
                return JsonResponse({'Status': False, 'Error': {'password': error_array}})
            else:
...
class ProductInfoView(APIView):
    ...
    def get(self, request: Request, *args, **kwargs):
        ...
        if category_id:
            query = query & Q(product__gategory_id=category_id)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if items_string:
            try:
                ...
                basket, _ = Order.objects.get_or_create(user_id=request.user_id, state='basket')
                objects_created = 0
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if items_string:
            ...
            if objects_deleted:
                ...
                return JsonResponse({'Status': True, 'Удалено объектов': deleted_count})
        return JsonResponse({'Status': False, 'Errors': 'Не укаазны все необходимые аргументы'})
            ...
            if request.data['id'].isdigit():
                try:
                    ...
                    if is_updated:
                        ...
                        send_email.delay(user_id=request.user.id)
                        return JsonResponse({'Status': True})
